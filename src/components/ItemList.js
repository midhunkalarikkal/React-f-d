...
const ItemList = ({ item }) => {
  return (
    <>
      {item.map((item, index) => 
        <div className="flex justify-between p-3 bg-slate-50 border-b-2 mt-1" key={index}>
          ...
          <div className="relative flex justify-center w-3/12">
            ...
            <button className="absolute bg-black text-green-400 font-bold text-md bottom-1 p-1 px-2 rounded-lg">Add</button>
          </div>
        </div>
      )}
    </>
